...
import { Meta, defineCollection, defineConfig } from "@content-collections/core";
...
function addMetaToVFile(meta: Meta): Pluggable {
  const relativeDirectoryPath = path.join(postDirectory, meta.directory);
  const _meta = {
    ...meta,
    relativeDirectoryPath,
    relativeFilePath: path.join(relativeDirectoryPath, meta.fileName),
  };
...
type MdxParams = { meta: Meta; content: string };

async function compileMdx({ meta, content }: MdxParams) {
  const files: Record<string, string> = {};

  const cmpPath = path.join(postDirectory, meta.directory, "components");
  if (existsSync(cmpPath)) {
    const components = await fs.readdir(cmpPath);
    for (const component of components) {
      if (component.endsWith(".tsx")) {
        const content = await fs.readFile(path.join(cmpPath, component), "utf-8");
        files[`./components/${component.replace(/\$.tsx$/, "")}`] = content;
      }
    }
  }
...
const posts = defineCollection({
  ...
  transform: async (post, ctx) => {
    const mdx = await ctx.cache({ meta: post._meta, content: post.content }, compileMdx);
