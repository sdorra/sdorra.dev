...
In [Next.js](https://nextjs.org/) we can use the [next/image component](https://nextjs.org/docs/api-reference/next/image) to avoid this bad user experience.
`next/image` automatically optimized our images in three ways to reduce the slow loading times:
...
This wont work, because [Next.js](https://nextjs.org/) does some magic on the import of the static image.
...
which does not know about the import magic for images.
...
This eliminates the need for static import and treats our image like a remote image.
...
The plugin will now find each image in markdown content and pass its dimensions to the image component.
...
And that's it, we are using now `next/image` to render our markdown images.
...
If these two things are not so important to you, this is the right approach for you.
...
Before we write write the plugin,
we should understand how the markdown processing works in [contentlayer](https://www.contentlayer.dev/).

[contentlayer](https://www.contentlayer.dev/) uses [remark](https://github.com/remarkjs/remark) to parse the markdown in an [mdast](https://github.com/syntax-tree/mdast).
...
* tree: The whole `hast`
* file: The parsed file which path and content
* done: function which should be called if our plugin is finished
...
First we have to find the real path of the image.
Fortunately, contentlayer stores information about the location of our Markdown file in the `data` field of `file`.
...
Now we have all the information in place, we can update the properties of the `img` tag.
...
<Notification>
  ...
  As we have seen in the <a href="#replace-img-with-next-image" className="underline">rehype-img-size method</a>.
</Notification>
...
For a more complete example have a look at the [source of this blog](https://github.com/sdorra/sdorra.dev/blob/main/lib/static-images.ts).
