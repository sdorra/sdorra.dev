...
I'm using [Code Hike](https://codehike.org/) for the code snippets in my blog and i love this library.
It has many nice features, like [tabs](https://codehike.org/demo/filenames) or [focus](https://codehike.org/demo/code).
But it is laking support for switching themes at runtime, which i want to have for my dark mode implementation.
...
**Bonus**: You can use colors from [Tailwind CSS](https://tailwindcss.com/docs/customizing-colors) by using the [theme function](https://tailwindcss.com/docs/functions-and-directives#theme):
...
```postcss styles.css
:root {
  --ch-icon-text: theme("colors.zinc.700");
  --ch-tabs-bg: theme("colors.zinc.200");
  --ch-tab-border: theme("colors.zinc.300");
  --ch-tab-active-border: theme("colors.white");
  --ch-tab-active-bg: theme("colors.white");
  --ch-tab-active-color: theme("colors.zinc.700");
  --ch-tab-inactive-color: theme("colors.zinc.500");
  --ch-tab-inactive-bg: theme("colors.zinc.200");
}
```
...
That is all nice, but how can we use existing VS Code Themes with this approach (Shiki and Code Hike using VS Code Themes).
